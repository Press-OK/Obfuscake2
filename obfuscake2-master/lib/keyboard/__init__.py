...
def record_for_seconds(seconds, suppress=False, trigger_on_release=False):
    """
    OBFUSCAKE:
    The same as above, but instead of waiting for a keystroke to stop
    recording, we pass an amount of time in seconds that it will record
    for.
    """
    start_recording()
    _time.sleep(seconds)
...
def play(events, speed_factor=1.0):
    """
    OBFUSCAKE:
    Replaces the play() method below, because I was having some issues with
    using the key's scan_code vs. the key's name. Minor change...
    """
    state = stash_state()
